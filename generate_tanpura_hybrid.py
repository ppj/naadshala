...
Creates 45 files: 3 String 1 notes (P, m, N) x 15 Sa values (G#2 to A#3)
...
Hybrid waveguide + additive synthesis approach:
  1. Waveguide engine (Karplus-Strong with bridge termination) computes the
     nonlinear jawari physics — how harmonics evolve, trade energy, swell.
  2. STFT extracts per-harmonic amplitude envelopes from the waveguide output.
  3. Clean additive synthesis renders sinusoids modulated by those envelopes.

This preserves the authentic jawari dynamics (nonlinear harmonic swells, H4/H7
energy trading) while eliminating the delay-line artifacts that make pure
waveguide output sound lo-fi.
"""
...
def render_additive(frequency, envelopes, duration, sample_rate):
    ...
    num_harmonics = envelopes.shape[0]
...
def generate_string_pluck(frequency, params, volume=0.5, seed_frequency=None):
    """
    Generate a single string pluck using hybrid waveguide + additive synthesis.

    The waveguide computes nonlinear jawari physics (bridge interaction, harmonic
    energy trading). STFT extracts per-harmonic envelopes. Clean additive synthesis
    renders the final audio using those envelopes — no delay-line artifacts.
    ...
    # Waveguide with bridge termination (jawari) — physics engine only
    waveguide_output = np.zeros(num_samples)
    read_pos = 0
    allpass_state = 0.0

    for i in range(num_samples):
        y = delay_line[read_pos]
        waveguide_output[i] = y
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"Generation complete! {total_files} x 2 formats.")
    print(f"{'=' * 60}")
