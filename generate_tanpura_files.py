...
JAWARI_BRIDGE_HEIGHT = 0.15    # lower bridge distance from rest (lower = more jawari)
JAWARI_JIVA_HEIGHT = 0.20      # upper bridge (jiva thread) — generates even harmonics (H4)
...
def generate_string_pluck(
    ...
    # Each string dips after pluck then rises as jawari energy builds through
    # bridge interaction, matching the real tanpura's visible amplitude swells.
    # Lower frequency strings have more pronounced swells (more bridge contact).
    # Shape: gamma-like curve t^2 * exp(-2t/peak), peaking at SWELL_PEAK.
    swell_depth = JAWARI_SWELL_DEPTH * min((130.0 / frequency) ** 0.5, 1.5)
    swell_b = 2.0 / JAWARI_SWELL_PEAK
